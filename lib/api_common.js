...
var AccessToken = function (accessToken, expiresIn) {
...
API.prototype.getAccessToken = function (callback) {
  ...
  this.request(url, {dataType: 'json'}, wrapper(function (err, data) {
    ...
    var token = AccessToken(data.access_token, data.expires_in);
...
API.prototype.preRequest = function (method, args) {
  ...
  that.getToken(function (err, token) {
    ...
    if (token) {
      // 暂时保存token
      that.token = AccessToken(token.accessToken, token.expiresIn);
...
API.prototype.getLatestToken = function (callback) {
  ...
  that.getToken(function (err, token) {
    ...
    // 有token
    if (token) {
      callback(null, AccessToken(token.accessToken, token.expiresIn));
    } else {
